...
ansible-playbook create_configure_vars_file.yml -i hosts
...
if [ "$1" == "--help" -o "$1" == "-h" ] ; then
   ...
   exit 0
     ...
     if [ -n "$controller_node_ip" ]; then
        controller_node_host_name="tvault-controller-$controller_node_ip"
        ansible-playbook $BASE_DIR/site.yml --vault-password-file ${BASE_DIR}/vars/vault_pass.txt -e "guest_name=$controller_node_host_name floating_ipaddress=${controller_node_ip} nodetype=controller" -i ${BASE_DIR}/hosts
        if [ $? -eq 0 ] ; then
